...
exports.VolunteerRolesService = void 0;
const common_1 = require("@nestjs/common");
const supabase_service_1 = require("../supabase/supabase.service");
let VolunteerRolesService = class VolunteerRolesService {
    ...
    async findAll(campaignId, status) {
        ...
        if (applicationsRes.error)
            throw new common_1.BadRequestException(applicationsRes.error.message);
        const campaignMap = new Map((campaignsRes.data ?? []).map((c) => [c.id, c]));
        const appsByRole = {};
        for (const a of applicationsRes.data ?? []) {
            if (!appsByRole[a.role_id])
                appsByRole[a.role_id] = [];
    ...
    async findOne(id) {
        const { data: role, error } = await this.db
            ...
            .single();
        if (error || !role)
            throw new common_1.NotFoundException(`Volunteer role ${id} not found`);
        const [campaignRes, applicationsRes] = await Promise.all([
            role.campaign_id
                ? this.db.from('bh_campaigns').select('id, title, type, status, org_id').eq('id', role.campaign_id).maybeSingle()
...
exports.VolunteerRolesService = VolunteerRolesService;
exports.VolunteerRolesService = VolunteerRolesService = __decorate([
    (0, common_1.Injectable)(),
    __metadata("design:paramtypes", [supabase_service_1.SupabaseService])
], VolunteerRolesService);
//# sourceMappingURL=volunteer-roles.service.js.map